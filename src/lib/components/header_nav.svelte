...
<div class:swap-active={scrollY > 32 && title} class="swap order-last hidden lg:inline-grid ">
	<button
		...
		class="swap-on btn btn-ghost text-base font-normal normal-case transition-all duration-200 border-b border-gray-500 -skew-x-[15deg] translate-x-[0.125rem]"
	>
		<span class="px-4 skew-x-[15deg] px-2 -translate-x-[0.125rem]">{title}</span>
	</button>
	<ul class:hidden={scrollY > 64 && title} class="swap-off menu menu-horizontal p-0">
		{#each nav as { text, link, children }}
			{#if link && !children}
				<li>
					<a
						class="rounded-btn border-b border-gray-500 -skew-x-[15deg] translate-x-[0.125rem]"
						class:font-bold={link === path}
						href={link}
					>
						<span class="skew-x-[15deg] px-2 -translate-x-[0.125rem]">{text}</span>
					</a>
				</li>
			{:else if children}
				<li>
